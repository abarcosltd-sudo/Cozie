...
export const getAvailableUsers = async (req, res, next) => {
  ...
  try {
    ...
    for (const doc of usersSnapshot.docs) {
      ...
      const displayName = userData.fullname || userData.displayName || userData.username || 'User';
...
export const getConversations = async (req, res, next) => {
  ...
  try {
    ...
    for (const doc of conversationsSnapshot.docs) {
      ...
      if (!convData.otherUserId) continue;
      
      conversations.push({
        id: doc.id,
        otherUserId: convData.otherUserId,
        name: convData.otherUserName || 'User',
        avatar: convData.otherUserAvatar || null,
        ...
        isOnline: convData.otherUserOnline || false
      });
    }
...
export const deleteMessage = async (req, res, next) => {
  ...
  try {
    if (!conversationId || !messageId) {
      return res.status(400).json({ success: false, message: 'Conversation ID and Message ID are required' });
    }
